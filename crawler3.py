#!/usr/bin/env python3
# Github Fork
# Original: anoopby/PythonCrawler
# ForkRepo: mhenes/PythonCrawler
# Converted to Python3 with 2to3 tool
import datetime
import sys
if len(sys.argv)>=2:
    keyword=sys.argv[1]
...
def checkRobotTxt(url):
    if url in robotUrlDict:
        ...
        rob_url=urllib.parse.urljoin(domain,"robots.txt")
        try:
            rerp.fetch(rob_url)
...
def getBingResult(userinput):
    ...
    headers = {'Authorization': 'Basic ' + base64.decodebytes(encAccountKey)}
    req = urllib.request.Request(bingUrl, headers = headers)
...
def getGoogleResults(userinput):
	...
	while (fetch):
	    ...
	    userinput = urllib.parse.urlencode ({'q':userinput})
	    search_string=searchAPI+userinput+'&start='
	    fpgcount += 1
	    req = urllib.request.Request(search_string+str(8*fpgcount))
...
def fetchWebPAge(url):
	try:
		...
		for link in br.links():
			new_url = urllib.parse.urljoin(link.base_url,link.url)
			base = urllib.parse.urlparse(new_url).hostname
